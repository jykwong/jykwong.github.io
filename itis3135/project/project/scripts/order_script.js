...
$(document).ready(function() {
    ...
    menu_click = function(clicked_id) {
        ...
        for(let i = 0; i < menuLength; i++) {
            if (menuItems[i].name === clicked_id) {
                ...
                subTotal += amt; //adds amt variable to subtotal cost
                console.log("total: " + subTotal); //for testing-- show new total

                string += menuItems[i].name + "... $" + menuItems[i].price + "<br>"; //append this to order details for each item selected
            }
        }
        totalCost = subTotal + (subTotal*salesTax); //calculate total
    ...
    reset_order = function() { //method attached to a button to reset user's order
        string = "";
